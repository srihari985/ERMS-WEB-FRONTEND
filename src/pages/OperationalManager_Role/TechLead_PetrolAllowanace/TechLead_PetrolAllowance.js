...
const TechLead_PetrolAllowance = () => {
  ...
  if (date) {
    ...
    try {
      if (dateType === "year") {
        ...
        fetchUrl = `${baseUrl}/api/operationTechLead/perolAllowance/totalAmountAndKmPerYear/${year}`;
        filterKey = year; // Filter rows by year
      } else if (dateType === "month") {
        const yearMonth = format(date, "yyyy/MM");
        fetchUrl = `${baseUrl}/api/operationTechLead/perolAllowance/totalAmountAndKmPerMonth/${yearMonth}`;
        filterKey = format(date, "yyyy-MM"); // Filter rows by year and month
      } else if (dateType === "date") {
        const fullDate = format(date, "yyyy-MM-dd");
        fetchUrl = `${baseUrl}/api/operationTechLead/perolAllowance/totalAmountAndKmPerDay/${fullDate}`;
...
const fetchTableData = async () => {
  try {
    const baseUrl = process.env.REACT_APP_API_BASE_URL;
    const response = await fetch(`${baseUrl}/api/operationTechLead/perolAllowance/getBySalesId/${oTLId}`);
    const data = await response.json();

    // Ensure table data is always an array
    const formattedData = Array.isArray(data) ? data : [data];
    setTableData(formattedData);
    setFilteredTableData(formattedData); // Set as default filtered data
...
const handleSubmit = async (e) => {
  ...
  try {
    const response = await axios.post(`${baseUrl}/api/operationTechLead/perolAllowance/save/${oTLId}`, formData, {
...
const handleUpdate = async () => {
  ...
  const baseUrl = process.env.REACT_APP_API_BASE_URL;
  try {
   const response= await axios.patch(`${baseUrl}/api/operationTechLead/perolAllowance/update/${oTLId}`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    });
...
export default TechLead_PetrolAllowance;