...
const HrToSaleTeleTechOperRegistration = () => {
  ...
  const [secondFormItems, setSecondFormItems] = useState([]);
  ...
  const validationSchemaTab0 = yup.object().shape({
    ...
    relatedData: yup.string().required("Authority Id is required"),
  ...
  const handleSubmit = async (values) => {
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        const selectedId = values.relatedData; // This will capture the selected item ID
        // Determine API endpoint based on role
        let apiEndpoint;
        switch (values.role) {
          case 'MANAGER':
            apiEndpoint = `${baseUrl}/api/v1/humanResources/register/manager/${userId}`;
            break;
          case 'SALE_MANAGER':
              apiEndpoint = `${baseUrl}/api/v1/humanResources/register/saleManager/${selectedId}`;
              break;
          case 'OPERATION_TECH_LEAD':
              apiEndpoint = `${baseUrl}/api/v1/humanResources/register/operationTechLead/${selectedId}`;
              break;
          case 'SALES':
              apiEndpoint = `${baseUrl}/api/v1/humanResources/register/sales/${selectedId}`;
              break;          
          case 'TECHNICIAN':
              apiEndpoint = `${baseUrl}/api/v1/humanResources/register/technician/${selectedId}`;
              break;
          case 'TELECALLER':
              apiEndpoint = `${baseUrl}/api/v1/humanResources/register/telecaller/${selectedId}`;
              break;
          default:
        ...
        const response = await fetch(apiEndpoint, {
          ...
          body: formData,
          headers: {
            'Authorization': `Bearer ${managetoken}`, // Replace token with your auth token
          },
        });
  ...
  // Function to determine when to show second FormControl
  const handleRoleChange = (event, setFieldValue) => {
    const selectedRole = event.target.value;
    setFieldValue('role', selectedRole);
  
    // Show second FormControl for specific roles
    if (
      // selectedRole === 'MANAGER' ||
      selectedRole === 'SALE_MANAGER' ||
      selectedRole === 'OPERATION_TECH_LEAD' ||
      selectedRole === 'SALES' ||
 ...
 // Fetch data based on the selected role using Fetch API
const fetchDataForRole = async (role) => {
  try {
    let response;
    let data;

    // Check if the role falls under the same API group
    if (role === 'SALES' || role === 'TELECALLER' || role === 'TECHNICIAN') {
      // Call the same API for these roles
      response = await fetch(`${baseUrl}/api/v1/humanResources/getListOf/OpTechLead/ids`);
      if (!response.ok) {
        throw new Error('Failed to fetch data for Sales, Telecaller, and Technician');
      }
      data = await response.json();
    } 
    // Individual APIs for MANAGER, SALE_MANAGER, OPERATION_TECH_LEAD roles
    // else if (role === 'MANAGER') {
    //   response = await fetch(`${baseUrl}/api/v1/admin/getListOf/HumanResources/ids`);
    //   if (!response.ok) {
    //     throw new Error('Failed to fetch data for Manager');
    //   }
    //   data = await response.json();
    // } 
    else if (role === 'SALE_MANAGER') {
      response = await fetch(`${baseUrl}/api/v1/humanResources/getListOf/Manager/ids`);
      if (!response.ok) {
        throw new Error('Failed to fetch data for Sale Manager');
      }
      data = await response.json();
    } 
    else if (role === 'OPERATION_TECH_LEAD') {
      response = await fetch(`${baseUrl}/api/v1/humanResources/getListOf/SaleManager/ids`);
      if (!response.ok) {
        throw new Error('Failed to fetch data for Operation Tech Lead');
      }
      data = await response.json();
    }

    // Set the fetched items for the second FormControl
    if (data) {
      setSecondFormItems(data); // Assuming the response returns a list of items
    }
  } catch (error) {
...
<FormControl fullWidth>
                      ...
                      >
                        ...
                        <MenuItem value="SALE_MANAGER">SALE_MANAGER</MenuItem>
                        <MenuItem value="OPERATION_TECH_LEAD">OPERATION_TECH_LEAD</MenuItem>
                    ...
                    {/* Conditionally render second FormControl */}
                    {showSecondForm && (
                        <FormControl fullWidth>
                          <InputLabel>Select Authority Id</InputLabel>
                          <Select
                            label="Select Authority Id"
                            value={values.relatedData}
                            onChange={handleChange}
                            name="relatedData"
                            onBlur={handleBlur}
                            error={!!touched.relatedData && !!errors.relatedData}
                          >
                            {secondFormItems.map((item) => {
                              // Check for available ID fields based on role-specific response
                              const id = item.oTechnicalLeadId || item.salesManagerId || item.managersId || item.humanResourcesId; // Add any other role-specific IDs
                              return (
                                <MenuItem key={id} value={id}>
                                  {`${item.firstname} ${item.lastname} (${id})`} {/* Format: "firstname lastname (ID)" */}
                                </MenuItem>
                              );
                            })}
                          </Select>
                          {touched.relatedData && errors.relatedData && (
                            <Typography color="error" variant="body2">
                              {errors.relatedData}
                            </Typography>
                          )}
...
export default HrToSaleTeleTechOperRegistration;