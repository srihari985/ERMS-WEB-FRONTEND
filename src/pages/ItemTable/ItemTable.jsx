...
const fetchItemsByFormId = async () => {
  ...
  try {
    const response = await fetch(`http://192.168.29.219:8080/api/itemstable/getAllForms`);
    if (response.ok) {                                     
...
const handleAddItems = async (items) => {
  try {
    ...
    const response = await fetch(`http://192.168.29.219:8080/api/itemSalesTable/saveitems/${fId}/${sId}`, {
      method: 'POST',
      headers: {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://192.168.29.219:8080/api/itemstable/saveitems/${fId}`, {
        method: 'POST',                 
        headers: {
          'Content-Type': 'application/json',
        },
