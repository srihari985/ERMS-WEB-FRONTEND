...
const ItemTable = () => {
  ...
  const { fId } = useAuth();
  console.log("I am item tab fid :" + fId)
...
const fetchItemsByFormId = async () => {
  ...
  try {
    const response = await fetch(`http://localhost:8081/api/itemstable/getAllForms`);
    if (response.ok) {                                     
...
useEffect(() => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8081/api/itemstable/saveitems/${fId}`, {
        method: 'POST',                 
        headers: {
