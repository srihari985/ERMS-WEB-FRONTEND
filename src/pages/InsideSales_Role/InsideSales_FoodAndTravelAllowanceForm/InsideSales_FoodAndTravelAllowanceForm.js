...
const InsideSales_FoodAndTravelAllowance = () => {
  ...
  const [foodsearchDate, setFoodSearchDate] = useState('');
   ...
   const fetchTravelAllowanceData = async () => {
    try {
      ...
      const response = await fetch(`${baseUrl}/api/telecaller/travelAllowance/getAll`);
  ...
  const fetchFoodAllowanceData = async () => {
    try {
      const baseUrl=process.env.REACT_APP_API_BASE_URL
      const response = await fetch(`${baseUrl}/api/telecaller/foodAllowance/getAll`);
      const data = await response.json();                     
  ...
  const handleTravelSubmit = async (values, { setSubmitting, resetForm }) => {
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}/api/telecaller/travelAllowance/save/${telId}`, {
        method: 'POST',
        body: formData,
      });
  ...
  const handleFoodSubmit = async (values, { setSubmitting, resetForm }) => {
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}/api/telecaller/foodAllowance/save/${telId}`, {
        method: 'POST',
