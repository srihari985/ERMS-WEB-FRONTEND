  ...
  if (date) {
    ...
    try {
      if (dateType === "year") {
        ...
        fetchUrl = `${baseUrl}/api/telecaller/perolAllowance/totalAmountAndKmPerYear/${year}`;
        filterKey = year; // Filter rows by year
      } else if (dateType === "month") {
        const yearMonth = format(date, "yyyy/MM");
        fetchUrl = `${baseUrl}/api/telecaller/perolAllowance/totalAmountAndKmPerMonth/${yearMonth}`;
        filterKey = format(date, "yyyy-MM"); // Filter rows by year and month
      } else if (dateType === "date") {
        const fullDate = format(date, "yyyy-MM-dd");
        fetchUrl = `${baseUrl}/api/telecaller/perolAllowance/totalAmountAndKmPerDay/${fullDate}`;
...
const fetchTableData = async () => {
  try {
    const baseUrl = process.env.REACT_APP_API_BASE_URL;
    const response = await fetch(`${baseUrl}/api/telecaller/perolAllowance/getBySalesId/${telId}`);
    const data = await response.json();
...
const handleSubmit = async (e) => {
  ...
  try {
    const response = await axios.post(`${baseUrl}/api/telecaller/perolAllowance/save/${telId}`, formData, {
...
const handleUpdate = async () => {
  ...
  const baseUrl = process.env.REACT_APP_API_BASE_URL;
  try {
   const response= await axios.patch(`${baseUrl}/api/telecaller/perolAllowance/update/${telId}`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    });
