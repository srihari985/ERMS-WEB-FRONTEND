...
const InsideSales_CustomerList = () => {
  ...
  const handleEditSubmit = async () => {
    try {
      const response = await fetch(`${baseUrl}/api/telecaller/contactForm/update/${customerId}`, {
        method: 'PATCH',
        headers: {
  ...
  const handleGetData = async () => {
    try {
      const response = await fetch(`${baseUrl}/api/telecaller/contactForm/getAll/${telId}`);
      if (response.ok) {
  ...
  const handleCustomerSubmit = async () => {
    try {
      ...
      const response = await fetch(`${baseUrl}/api/telecaller/contactForm/save/${telId}?contactPerson=${contactPerson}&contactNumber=${contactNumber}&companyName=${companyName}&emailId=${emailid}&address=${address}&gstIn=${gstIn}`, {
        method: 'POST',
    ...
    >
      <Box display="flex" justifyContent="space-between" alignItems="center" marginTop="20px" marginBottom="20px">
        <Box display="flex" alignItems="center">
          {/* <IconButton onClick={() => navigate('/SalesDashboard')} sx={{ color: '#2e2a54' }}>
