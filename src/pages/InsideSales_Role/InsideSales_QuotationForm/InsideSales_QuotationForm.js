...
const handleDueDateChange = (e) => {
  ...
  const handleUpdate = async () => {
    ...
    try {
      // const fId = localStorage.getItem("fid");
      const baseUrl = process.env.REACT_APP_API_BASE_URL;
      const response = await fetch(`${baseUrl}/api/itemSalesTable/updateItems/${sId}/${AdId}/${AdId}?itIds=${itemIds}`, {
        method: 'PATCH',
...
const handleSaveQuotation = async () => {
  ...
  try {
    // const fId = localStorage.getItem("fid");
    const response = await fetch(`${baseUrl}/api/quotationList/createQuotation/${sId}/${AdId}/${AdId}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
