...
const InsideSales_AddParty = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${baseUrl}/api/telecaller/addParty/save/${telId}`,
        formData, 
        {
          headers: {
  ...
  const fetchAddPartyGetAll = async () => {
    try {
      ...
      const response = await fetch(
        `${baseUrl}/api/telecaller/addParty/getAll/${telId}`
      );
      if (!response.ok) {
  ...
  const handleDelete = async (adId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const baseUrl=process.env.REACT_APP_API_BASE_URL
          await axios.delete(`${baseUrl}/api/telecaller/addParty/${adId}`);
  ...
  const handleAddToQuotation = (party) => {
    ...
    navigate('/InsideSales_QuotationForm'); // Navigate without passing state
  };
