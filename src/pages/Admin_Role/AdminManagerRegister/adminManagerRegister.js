  ...
  const handleSubmit = async (values) => {
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        console.log('i am selected item id:'+selectedId)
        // Determine API endpoint based on role
        let apiEndpoint;
        switch (values.role) {    
          case 'ACCOUNTS':
            apiEndpoint = `${baseUrl}/api/v1/admin/register/accounts/${userId}`;
            break;
          case 'HUMAN_RESOURCES':
              apiEndpoint = `${baseUrl}/api/v1/admin/register/humanResources/${userId}`;
              break;    
          case 'MANAGER':
              apiEndpoint = `${baseUrl}/api/v1/admin/register/manager/${selectedId}`;
              break;                 
          case 'SALE_MANAGER':
              apiEndpoint = `${baseUrl}/api/v1/admin/register/saleManager/${selectedId}`;
              break;
          case 'OPERATION_TECH_LEAD':
              apiEndpoint = `${baseUrl}/api/v1/admin/register/operationTechLead/${selectedId}`;
              break;
          case 'SALES':
              apiEndpoint = `${baseUrl}/api/v1/admin/register/sales/${selectedId}`;
              break;
          case 'TECHNICIAN':
              apiEndpoint = `${baseUrl}/api/v1/admin/register/technician/${selectedId}`;
              break;
          case 'TELECALLER':
              apiEndpoint = `${baseUrl}/api/v1/admin/register/telecaller/${selectedId}`;
              break;
          default:
...
const fetchDataForRole = async (role) => {
  try {
    ...
    if (role === 'SALES' || role === 'TELECALLER' || role === 'TECHNICIAN') {
      ...
      response = await fetch(`${baseUrl}/api/v1/admin/getListOf/OpTechLead/ids`);
      if (!response.ok) {
    ...
    // Individual APIs for MANAGER, SALE_MANAGER, OPERATION_TECH_LEAD roles
      ...
      response = await fetch(`${baseUrl}/api/v1/admin/getListOf/Manager/ids`);
      if (!response.ok) {
      ...
      response = await fetch(`${baseUrl}/api/v1/admin/getListOf/SaleManager/ids`);
      if (!response.ok) {
...
<FormControl fullWidth>
                    ...
                    {showSecondForm && (
                        <FormControl fullWidth>
                          <InputLabel>Select Related Data</InputLabel>
                          <Select
                            label="Select Related Data"
