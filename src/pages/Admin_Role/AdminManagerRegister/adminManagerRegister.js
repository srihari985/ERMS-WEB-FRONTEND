...
const roles = ['MANAGER',"ACCOUNTS","SALE_MANAGER","HUMAN_RESOURCES","OPERATION_TECH_LEAD", 'SALES', 'TECHNICIAN' , 'TELECALLER'];
...
const AdminManagerRegister = () => {
  ...
  //       `${baseUrl}/api/v1/admin/register/manager/${userId}?firstname=${values.firstname}&lastname=${values.lastname}&email=${values.email}&role=${values.role}`,
  //       {
  //         method: 'POST',
  //         headers: {
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    try {
      ...
      switch (values.role) {
        case 'MANAGER':
          apiUrl = `${baseUrl}/api/v1/admin/register/manager/${userId}?firstname=${values.firstname}&lastname=${values.lastname}&email=${values.email}&role=${values.role}`;
          break;
        case 'ACCOUNTS':
          apiUrl = `${baseUrl}/api/v1/admin/register/accounts/${userId}?firstname=${values.firstname}&lastname=${values.lastname}&email=${values.email}&role=${values.role}`;
          break;              
        case 'SALE_MANAGER':
          apiUrl = `${baseUrl}/api/v1/management/register/saleManager/${userId}?firstname=${values.firstname}&lastname=${values.lastname}&email=${values.email}&role=${values.role}`;
          break;
