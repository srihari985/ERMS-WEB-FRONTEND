...
const Manager_FoodTravelAllowance = () => {
  ...
  const [foodsearchDate, setFoodSearchDate] = useState('');
   ...
   const fetchTravelAllowanceData = async () => {
    try {
      ...
      const response = await fetch(`${baseUrl}/api/manager/travelAllowance/getAll`);
  ...
  const fetchFoodAllowanceData = async () => {
    try {
      const baseUrl=process.env.REACT_APP_API_BASE_URL
      const response = await fetch(`${baseUrl}/api/manager/foodAllowance/getAll`);
      const data = await response.json();                     
  ...
  const handleTravelSubmit = async (values, { setSubmitting, resetForm }) => {
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}/api/manager/travelAllowance/save/${mId}`, {
        method: 'POST',
        body: formData,
      });
  ...
  const handleFoodSubmit = async (values, { setSubmitting, resetForm }) => {
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}/api/manager/foodAllowance/save/${mId}`, {
        method: 'POST',
...
export default Manager_FoodTravelAllowance;