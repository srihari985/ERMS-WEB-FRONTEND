  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        const response = await fetch(`${baseUrl}/api/manager/dailyReport/save/${mId}`, {
          method: 'POST',
          body: formData,
        });
        ...
        if (response.ok) {
          ...
          setTimeout(() => {
            navigate('/Manager_DailyReportList');
          }, 1000);
        } else {
    ...
    <Card sx={{ marginTop: '70px', marginLeft: '20px', marginRight: '20px' }}>
      <CardContent>
        <div style={{ marginBottom: '40px', display: 'flex', alignItems: 'center' }}>
          <IconButton onClick={() => navigate('/Manager_DailyReportList')} sx={{ marginRight: 1 }}>
            <ArrowBackIcon />
          </IconButton>
...
export default Manager_DailyReportForm;