  ...
  if (date) {
    ...
    try {
      if (dateType === "year") {
        ...
        fetchUrl = `${baseUrl}/api/manager/perolAllowance/totalAmountAndKmPerYear/${year}`;
        filterKey = year; // Filter rows by year
      } else if (dateType === "month") {
        const yearMonth = format(date, "yyyy/MM");
        fetchUrl = `${baseUrl}/api/manager/perolAllowance/totalAmountAndKmPerMonth/${yearMonth}`;
        filterKey = format(date, "yyyy-MM"); // Filter rows by year and month
      } else if (dateType === "date") {
        const fullDate = format(date, "yyyy-MM-dd");
        fetchUrl = `${baseUrl}/api/manager/perolAllowance/totalAmountAndKmPerDay/${fullDate}`;
...
const fetchTableData = async () => {
  try {
    const baseUrl = process.env.REACT_APP_API_BASE_URL;
    const response = await fetch(`${baseUrl}/api/manager/perolAllowance/getBySalesId/${mId}`);
    const data = await response.json();
...
const handleSubmit = async (e) => {
  ...
  try {
    const response = await axios.post(`${baseUrl}/api/manager/perolAllowance/save/${mId}`, formData, {
...
const handleUpdate = async () => {
  ...
  const baseUrl = process.env.REACT_APP_API_BASE_URL;
  try {
   const response= await axios.patch(`${baseUrl}/api/manager/perolAllowance/update/${mId}`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    });
...
export default Manager_PetrolAllowance;