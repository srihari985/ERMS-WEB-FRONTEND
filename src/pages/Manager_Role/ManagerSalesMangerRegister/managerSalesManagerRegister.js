  ...
  const handleSubmit = async (values) => {
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        switch (values.role) {
          case 'SALE_MANAGER':
              apiEndpoint = `${baseUrl}/api/v1/management/register/saleManager/${userId}`;
              break;                   
          case 'OPERATION_TECH_LEAD':
              apiEndpoint = `${baseUrl}/api/v1/management/register/operationTechLead/${selectedId}`;
              break;
          case 'SALES':
              apiEndpoint = `${baseUrl}/api/v1/management/register/sales/${selectedId}`;
              break;          
          case 'TECHNICIAN':
              apiEndpoint = `${baseUrl}/api/v1/management/register/technician/${selectedId}`;
              break;
          case 'TELECALLER':
              apiEndpoint = `${baseUrl}/api/v1/management/register/telecaller/${selectedId}`;
              break;
          default:
...
const fetchDataForRole = async (role) => {
  try {
    ...
    if (role === 'SALES' || role === 'TELECALLER' || role === 'TECHNICIAN') {
      ...
      response = await fetch(`${baseUrl}/api/v1/management/getListOf/OpTechLead/ids`);
      if (!response.ok) {
    ...
    // } 
      ...
      response = await fetch(`${baseUrl}/api/v1/management/getListOf/SaleManager/ids`);
      if (!response.ok) {
