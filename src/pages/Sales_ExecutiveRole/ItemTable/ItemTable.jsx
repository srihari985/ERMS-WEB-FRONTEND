...
const ItemTable = () => {
  ...
  const { sId, setitId, setSelectedAddItems, AdId } = useAuth();
  ...
  const handleAddItems = async (items) => {
    try {
      ...
      const contextFId = localStorage.getItem("fId");

  
      // Prepare the base API URL (normal or with quotationData)
      const apiUrl = quotationData
        ? `${baseUrl}/api/itemSalesTable/saveitems/${quotationData.adId}/${quotationData.fId}/${quotationData.saleId}` // If quotationData exists
        : `${baseUrl}/api/itemSalesTable/saveitems/${AdId}/${contextFId}/${sId}`; // Normal API call without quotationData
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (quotationData?.fId) {
        ...
        apiUrl = `${baseUrl}/api/itemstable/saveitems/${quotationData.fId}`;
      } else {
        // If only direct fId is provided, use this API
        apiUrl =` ${baseUrl}/api/itemstable/saveitems/${contextFId}`;
      }
