  ...
  const fetchQuotations = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Assuming each quotation has a `createdAt` field
        const sortedData = data.sort((a, b) => {
          // Extract the numeric part (after the last hyphen) from the quotationNumber
          const numA = parseInt(a.quotationNumber.split('-').pop(), 10);
          const numB = parseInt(b.quotationNumber.split('-').pop(), 10);
          
          // Compare the extracted numbers
          return numB - numA; // Descending order
        });
        setQuotations(sortedData); // Update quotations state with sorted data
        setFilteredQuotations(sortedData); // Update filtered quotations state with sorted data
      } else {
      ...
      switch (dateRange) {
        case "today":
          filteredData = filteredData.filter((q) => {
            ...
            console.log("Checking today's date:", quotationDate.toDateString(), " vs. ", now.toDateString());
        ...
        case "yesterday":
          ...
          filteredData = filteredData.filter((q) => {
            const quotationDate = new Date(q.quotationDate);
            console.log("Checking yesterday's date:", quotationDate.toDateString(), " vs. ", yesterday.toDateString());
            return quotationDate.toDateString() === yesterday.toDateString();
          });
        ...
        case "lastWeek":
          ...
          filteredData = filteredData.filter((q) => {
            const quotationDate = new Date(q.quotationDate);
            console.log("Checking last week's date:", quotationDate, " >= ", lastWeek);
        ...
        case "lastMonth":
          ...
          filteredData = filteredData.filter((q) => {
            const quotationDate = new Date(q.quotationDate);
            console.log("Checking last month's date:", quotationDate, " >= ", lastMonth);
            return quotationDate >= lastMonth;
          });
        ...
        case "last6Months":
          ...
          filteredData = filteredData.filter((q) => {
            const quotationDate = new Date(q.quotationDate);
            console.log("Checking last 6 months' date:", quotationDate, " >= ", last6Months);
            return quotationDate >= last6Months;
          });
          console.log("Filtered Data after 'last6Months':", filteredData);
          break;
        case "lastYear":
          ...
          filteredData = filteredData.filter((q) => {
            const quotationDate = new Date(q.quotationDate);
            console.log("Checking last year's date:", quotationDate, " >= ", lastYear);
            return quotationDate >= lastYear;
          });
          console.log("Filtered Data after 'lastYear':", filteredData);
          break;
        default:
          break;
      }
