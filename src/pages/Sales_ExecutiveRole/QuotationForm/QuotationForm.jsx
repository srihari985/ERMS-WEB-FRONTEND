...
const QuotationForm = () => {
  ...
  const { AdId, sId, setFid,fId: contextFId } = useAuth();
  ...
  const [terms, setTerms] = useState('');  // Define 'terms' state
  ...
  const fetchTableData = async () => {
    try {
      ...
      if (quotationData && quotationData.fId) {
        ...
        apiUrl = `${baseUrl}/api/itemSalesTable/getfinalItems/${contextFId}`; // Default API
      }
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (quotationData && quotationData.fId) {
        ...
        apiUrl = `${baseUrl}/api/itemSalesTable/updateItems/${sId}/${AdId}/${contextFId}?itIds=${itemIds}`;
      }
  ...
  const handleSubmit1 = async () => {
    ...
    try {
      ...
      if (quotationData && quotationData.fId) {
        ...
        // Use fId from localStorage or other context if accessing directly
        apiUrl = `${baseUrl}/api/itemsTotal/update/${contextFId}?additionalCharges=${additionalCharges}&description=${description}`;
      }
      const response = await fetch(apiUrl,
        {
    ...
    <Grid item xs={6}>
      ...
      {showNotes ? (
        <TextField
          label="Notes"
          fullWidth
          value={notes}  // Ensure 'notes' state is defined
          onChange={(e) => setNotes(e.target.value)}  // Correctly updating state
      ...
      <Grid item xs={12} sx={{ marginTop: "10px" }}>
        {showTerms ? (
          <TextField
            ...
            fullWidth
            value={terms}  // Ensure 'terms' state is defined
            onChange={(e) => setTerms(e.target.value)}  // Correctly updating state
