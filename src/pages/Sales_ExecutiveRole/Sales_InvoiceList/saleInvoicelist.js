...
export default function SalesInvoiceList() {
  ...
  useEffect(() => {
    const fetchTechOptions = async () => {
      try {
        const response = await fetch(`${baseUrl}/api/v1/sales/getListOf/OpTechLead/ids`);
        if (response.ok) {
  ...
  const fetchQuotations = async () => {
    try {
      const response = await fetch(`${baseUrl}/api/invoice/getAll/sales/${sId}`);
      const data = await response.json();
...
// API Call function for year, month, and/or salesId
const handleDateAPI = async (year, month, salesId) => {
  let endpoint = '';
  ...
  if (year && month) {
    ...
    endpoint = `${baseUrl}/api/v1/sales/totalPaidInMonth/${date}/${salesId}`;
  }                       
  // Check if only the year is provided to call the year-based API
    ...
    endpoint = `${baseUrl}/api/v1/sales/totalPaidInYear/${date}/${salesId}`;
  } 
  // If no date components are provided, call the salesId-only API
  else if (!year && !month && salesId) {
    endpoint = `${baseUrl}/api/v1/sales/totalPaid/${salesId}`;
  } else {
    console.error("Invalid date selection or missing salesId");
    return;
  }
...
useEffect(() => {
  if (salesId && selectedYear && selectedMonth) {
    // Condition 3: salesId, year & month
    handleDateAPI(selectedYear, selectedMonth, salesId);
  } else if (salesId && selectedYear) {
    // Condition 2: salesId & year (without month)
    handleDateAPI(selectedYear, null, salesId);
  } else if (salesId) {
    // Condition 1: Only salesId
    handleDateAPI(null, null, salesId);
  }
}, [selectedYear, selectedMonth, salesId]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`${baseUrl}/api/ticket/save/${selectedTicket.fId}/${ticketData.techLeadIds}`, {
        method: "POST",
