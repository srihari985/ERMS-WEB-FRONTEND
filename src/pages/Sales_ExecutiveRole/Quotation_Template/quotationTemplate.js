...
;  // Outputs: "Thirty Eight Thousand Five Hundred Forty Five and Fifty Paise"
...
const QuotationTemplate = () => {
  ...
  const { orgId } = useAuth();
  const orgitemp = orgId;
  console.log("I am template orgId :"+ orgitemp);
  ...
  const handleEdit=() =>{
    navigate('/QuotationForm', { state: { quotationData } });
  }
  ...
  const fetchPartyById = async () => {
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}/api/addParty/getPartyById/${quotationData.adId}`);
      if (response.ok) {
  ...
  const fetchTableData = async () => {
    try {
      const response = await fetch(`${baseUrl}/api/itemSalesTable/getfinalItems/${quotationData.fId}`); // Replace with your API endpoint
      if (response.ok) {
        const data = await response.json();
  ...
  const fetchAmountsData = async () => {
    try {
      const response = await fetch(`${baseUrl}/api/itemsTotal/getTotals/${quotationData.fId}`); // Replace with your API endpoint
      if (response.ok) {
        const data = await response.json();
  ...
  const handleConvertToInvoice = async () => {
    ...
    try {
      ...
      formData.append("grandTotal", quotationData.grandTotal); // Add the grandTotal as form data
  
      // Send the quotation data to convert to invoice
      const response = await fetch(
       ` ${baseUrl}/api/invoice/save/sales/${quotationData.fId}`, // Replace with your endpoint
        {
          method: "POST",
      ...
      if (response.ok && result.status === "accounts") {
        // const result = await response.json();
        toast.success('Quotation send to Accounts successfully', {
          position: "top-right",
          autoClose: 2000,
        });
      } 
      
      if(response.ok && result.status === "notAccounts") {
        // const errorText = await response.text();
       
        toast.success('Faild to send accounts', {
          position: "top-right",
          autoClose: 2000,
        });
      }
    ...
    >
      ...
      >
        ...
        <Box display="flex"  sx={{ marginBottom: '20px' }}>
          ...
          >
            Quotation/ Estimate #{' '}
            <span style={{ color: '#1976d2' }}>{quotationData.quotationNumber}</span>
          </Typography>
        </Box>
             ...
             <Typography
              ...
              >
                      ...
                      >
                        <Grid item>
                          <Grid container justifyContent="space-between">
                            ...
                            >
                              {/* <span>:</span> */}
                               <strong>{quotationData.quotationNumber}</strong>
                            </Typography>
                          </Grid>
                        </Grid>
                        <Grid item>
                          <Grid container justifyContent="space-between">
                            ...
                            >
                              {/* <span>:</span> */}
                               <strong>{quotationData.quotationDate}</strong>
                            </Typography>
                          </Grid>
                        </Grid>
                ...
                {saleItems.map((item, index) => (
                  <TableRow key={index}>
                    {/* style={{ border: "none", textAlign: "center", padding: "4px" }} */}
                ...
                {/* <Table sx={{ borderCollapse: "collapse" }} style={{
                    backgroundColor: "#F9ADA4",
                    marginTop: "10px",
                    marginBottom: "9px",
                    width: "100%",
                  ...
                  <TableHead >
                    <TableRow>
                      <th sx={{  marginRight: "10px",textAlign: "center",fontSize: "15px", backgroundColor: "#F9ADA4",fontWeight: "bold", width: "5%"}}></th>
                      <th sx={{ border: "1px solid #ddd", padding: "8px",textAlign: "center",fontSize: "15px", backgroundColor: "#F9ADA4",fontWeight: "bold", width: "25%"}}>SUBTOTAL</th>
                      <th sx={{  padding: "8px",textAlign: "center",fontSize: "15px", backgroundColor: "#F9ADA4",fontWeight: "bold", width: "10%"}}></th>
                      <th sx={{  padding: "8px",textAlign: "center",fontSize: "15px", backgroundColor: "#F9ADA4",fontWeight: "bold", width: "5%" }}></th>
                      <TableCell sx={{ border: "1px solid #ddd", padding: "8px",textAlign: "center",fontSize: "15px", backgroundColor: "#F9ADA4",fontWeight: "bold", width: "11%" }}>{amounts.totalAmount}</TableCell>
                      <TableCell sx={{ border: "1px solid #ddd", padding: "8px",textAlign: "center",fontSize: "15px", backgroundColor: "#F9ADA4",fontWeight: "bold", width: "11%" }}>{amounts.totalDiscount}</TableCell>
                      <TableCell sx={{ border: "1px solid #ddd", padding: "8px",textAlign: "center",fontSize: "15px", backgroundColor: "#F9ADA4",fontWeight: "bold", width: "11%" }}>{amounts.totalTax}</TableCell>
                      <TableCell sx={{ border: "1px solid #ddd", padding: "8px",textAlign: "center",fontSize: "15px", backgroundColor: "#F9ADA4",fontWeight: "bold", width: "16%" }}>{amounts.finalAmount}</TableCell>
                    </TableRow>
                  </TableHead>
                </Table> */}
