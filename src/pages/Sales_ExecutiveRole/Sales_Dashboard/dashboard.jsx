...
const SalesDashboard = () => {
  ...
  const { sId,setSalescount } = useAuth();
  ...
  useEffect(() => {
    ...
    const fetchQuotations = async () => {
      try {
        ...
        const sortedData = data.sort((a, b) => new Date(b.quotationDate) - new Date(a.quotationDate)); 
        setQuotations(sortedData);  // Set API data to quotations state
      } catch (error) {
  ...
  const fetchQuotationsCount = async () => {
    try {
      const response = await fetch(`${baseUrl}/api/v1/sales/quotation/count/${sId}`);
      const data = await response.json();
  ...
  const fetchInvoiceCount = async () => {
    try {
      const response = await fetch(`${baseUrl}/api/v1/sales/invoices/count/${sId}`);
      const data = await response.json();
   ...
   const fetchTotalCustomersCount = async () => {
    try {
      const response = await fetch(`${baseUrl}/api/contactForm/customerList/count/${sId}`);
      const data = await response.json();
