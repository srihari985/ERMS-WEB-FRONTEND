...
const TechnicianDashboard = () => {
  ...
  const { userId: technicianId ,settechCount} = useAuth();
  ...
  const fetchNewTicketsCount = async () => {
    try {
      const response = await fetch(`${baseUrl}/api/v1/technician/getNewTickets/count/${technicianId}`);
      const data = await response.json();
  ...
  const fetchTotalCount = async () => {
    try {
      const response = await fetch(`${baseUrl}/api/v1/technician/getTotalTickets/count/${technicianId}`);
      const data = await response.json();
    ...
    const fetchCompletedTicketCount = async () => {
      try {
        const response = await fetch(`${baseUrl}/api/v1/technician/getCompletedTickets/count/${technicianId}`);
        const data = await response.json();
   ...
   const fetchTotalCustomersCount = async () => {
    try {
      const response = await fetch(`${baseUrl}/api/contactForm/customerList/count/${technicianId}`);
      const data = await response.json();
      ...
      >
        <CardContent>
          ...
          <Table  stickyHeader>
            <TableHead>
              <TableRow>
                {["S.NO","Company Name", "Invoice Number", "TechnicianId", "Address", "StartDate", "EndDate","Status"].map((header) => (
                  <TableCell
                    key={header}
                    style={{
                      fontWeight: "bold",
    ...
    <Grid container spacing={3}>
      <Grid item xs={12} sm={3}>
        <Card
          ...
          onClick={() => navigate('/TechnicianNewTickets')}
        >
      ...
      <Grid item xs={12} sm={3}>
        <Card
          ...
          onClick={() => navigate('/TechnicianTicketsHistory')}
        >
          <CardContent sx={{ textAlign: 'center' }}>
      ...
      <Grid item xs={12} sm={3}>
        <Card
          ...
          onClick={() => navigate('/TechnicianCompletedTicketsList')}
        >
          <CardContent sx={{ textAlign: 'center' }}>
