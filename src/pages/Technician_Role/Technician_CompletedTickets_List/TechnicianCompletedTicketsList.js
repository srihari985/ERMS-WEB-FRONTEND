...
export default function TechnicianCompletedTicketsList() {
  ...
  const fetchNewTicketsList = async () => {
    try {
      const response = await fetch(`${baseUrl}/api/v1/technician/getCompletedTickets/${technicianId}`);
  ...
  return (
    ...
    >
      ...
      <Table>
        <TableHead>
          <TableRow>
            {["S.No", "Company Name", "Invoice Number", "Technician Id", "Address", "Start Date", "End Date", "Status"].map((header) => (
              <TableCell
                key={header}
                style={{
                  fontWeight: "bold",
