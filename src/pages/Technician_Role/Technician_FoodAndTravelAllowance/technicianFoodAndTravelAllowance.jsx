...
const TechnicianFoodTravelAllowance = () => {
  ...
  const [foodsearchDate, setFoodSearchDate] = useState('');
   ...
   const fetchTravelAllowanceData = async () => {
    try {
      ...
      const response = await fetch(`${baseUrl}/api/technician/travelAllowance/getAll`);
  ...
  const fetchFoodAllowanceData = async () => {
    try {
      const baseUrl=process.env.REACT_APP_API_BASE_URL
      const response = await fetch(`${baseUrl}/api/technician/foodAllowance/getAll`);
      const data = await response.json();                     
  ...
  const handleTravelSubmit = async (values, { setSubmitting, resetForm }) => {
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}/api/technician/travelAllowance/save/${tId}`, {
        method: 'POST',
        body: formData,
      });
  ...
  const handleFoodSubmit = async (values, { setSubmitting, resetForm }) => {
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}/api/technician/foodAllowance/save/${tId}`, {
        method: 'POST',
