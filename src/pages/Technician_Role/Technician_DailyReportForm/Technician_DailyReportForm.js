...
const Technician_DailyReportForm = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        const response = await fetch(`${baseUrl}/api/technician/dailyReport/save/${tId}`, {
          method: 'POST',
          body: formData,
        });
        ...
        if (response.ok) {
          ...
          setTimeout(() => {
            navigate('/Technician_DailyReportList');
          }, 1000);
        } else {
    ...
    <Card sx={{ marginTop: '70px', marginLeft: '20px', marginRight: '20px' }}>
      <CardContent>
        <div style={{ marginBottom: '40px', display: 'flex', alignItems: 'center' }}>
          <IconButton onClick={() => navigate('/Technician_DailyReportList')} sx={{ marginRight: 1 }}>
            <ArrowBackIcon />
          </IconButton>
...
export default Technician_DailyReportForm;