  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://192.168.29.219:8080/api/v1/management/register/saleManager/${userId}?firstname=${values.firstname}&lastname=${values.firstname}&email=${values.email}&role=${values.role}`,
        { 
          method: 'POST',
          headers: {
