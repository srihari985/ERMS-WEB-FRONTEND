...
const QuotationForm = () => {
  ...
  const fetchPartyById = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8081/api/addParty/getPartyById/${selectedParty}`);
      if (response.ok) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8081/api/form/save/${selectedParty}/${sId}`, {
        method: 'POST',
        headers: {
