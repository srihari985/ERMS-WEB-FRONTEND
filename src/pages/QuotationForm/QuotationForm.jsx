...
const QuotationForm = () => {
  ...
  const fetchPartyById = async () => {
    ...
    try {
      const response = await fetch(`http://192.168.29.219:8083/api/addParty/getPartyById/${selectedParty}`);
      if (response.ok) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`http://192.168.29.219:8083/api/form/save/${selectedParty}/${sId}`, {
        method: 'POST',
        headers: {
...
const getFormattedDate = () => {
  ...
  const fetchTableData = async () => {
    try {
      ...
      const response = await fetch(`http://192.168.29.219:8083/api/itemSalesTable/getSelectedItems/${sId}/${saleDate}?itemIds=${itId}`); // Replace with your API endpoint
      if (response.ok) {
        const data = await response.json();
