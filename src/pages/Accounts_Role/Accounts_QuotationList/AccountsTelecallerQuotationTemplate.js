...
const AccountsTelecallerQuotationTemplate = () => {
  ...
  const apiEndpoints = {
    SaleInvoice: `/api/invoice/convert/telecaller/${quotationData.telId}/`,
    DC: "/api/telecaller/DCInvoice/save/",
    ProformaInvoice: "/api/telecaller/proFormaInvoice/save/",
  };
  ...
  const fetchPartyById = async () => {
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}/api/telecaller/addParty/getPartyById/${quotationData.adId}`);
      if (response.ok) {
  ...
  const fetchTableData = async () => {
    try {
      const response = await fetch(`${baseUrl}/api/telecaller/itemSalesTable/getfinalItems/${quotationData.fId}`); // Replace with your API endpoint
      if (response.ok) {
        const data = await response.json();
  ...
  const fetchAmountsData = async () => {
    try {
      const response = await fetch(`${baseUrl}/api/telecaller/itemsTotal/getTotals/${quotationData.fId}`); // Replace with your API endpoint
      if (response.ok) {
        const data = await response.json();
 ...
 // Handle the API call to convert the quotation to an invoice
 const handleConvertToInvoice = async (type) => {
  // Show confirmation dialog
  const { isConfirmed } = await Swal.fire({
    title: "Are you sure?",
    text: `Do you want to convert this quotation to a ${type}?`,
    icon: "warning",
  ...
  const apiEndpoint = apiEndpoints[type]; // Get the endpoint for the selected invoice type

  if (!apiEndpoint) {
    toast.error("Invalid invoice type selected.", {
      position: "top-right",
      autoClose: 2000,
    });
    setIsLoading(false);
  ...
  try {
    ...
    // formData.append("invoiceType", type); // Add selected invoice type to form data

    // Send the quotation data to convert to the selected invoice type
    const response = await fetch(
      `${baseUrl}${apiEndpoint}${quotationData.fId}`, // Use the appropriate endpoint for the selected invoice type
      {
        method: "POST",
    ...
    if (response.ok && result.status === "invoice") {
    ...
    if (response.ok && result.status === "notConfirm") {
      toast.success(`Quotation converted to ${type} successfully`, {
        position: "top-right",
        autoClose: 2000,
      });
    }
  ...
  const handleMenuItemClick = (type) => {
    setSelectedInvoiceType(type); // Set the selected invoice type
    handleConvertToInvoice(type);
    handleMenuClose(); // Close the dropdown after selection
 // Call the API when an invoice type is selected
  };
      ...
      >
        ...
        <Box display="flex"  sx={{ marginBottom: '20px' }}>
          <ArrowBackIcon
            ...
            // onClick={ () => navigate("/Accounts_QuotationList")}
            onClick={() => navigate(-1)}
          />
          <Typography
            variant="body1"
        ...
        sx={{
          "& .MuiMenu-paper": {
            ...
            boxShadow: "0 4px 8px rgba(0, 0, 0, 0.15)", // Menu shadow
            padding: "10px", // Padding around the menu items
          },
        }}
      >
        <MenuItem
          onClick={() => handleMenuItemClick("SaleInvoice")}
          sx={{
            color: "#388e3c", // Green text color
            fontWeight: "bold",
            "&:hover": {
              backgroundColor: "#d0f8c0", // Light green background on hover
              color: "#1b5e20", // Darker green text color on hover
            },
          }}
        >
        ...
        <MenuItem
          onClick={() => handleMenuItemClick("DC")}
          sx={{
            color: "#0288d1", // Blue text color
            fontWeight: "bold",
            "&:hover": {
              backgroundColor: "#b3e5fc", // Light blue background on hover
              color: "#01579b", // Darker blue text color on hover
            },
          }}
        >
          Convert to DC
        ...
        <MenuItem
          onClick={() => handleMenuItemClick("ProformaInvoice")}
          sx={{
            color: "#f57c00", // Orange text color
            fontWeight: "bold",
            "&:hover": {
              backgroundColor: "#ffe0b2", // Light orange background on hover
              color: "#e65100", // Darker orange text color on hover
            },
          }}
        >
...
export default AccountsTelecallerQuotationTemplate;