...
const AccountsTelecallerDCTemplate = () => {
  ...
  // Access the passed state (dcData)
  const { dcData } = location.state || {};
  ...
  // const handleEdit=() =>{
  //   navigate('/QuotationForm', { state: { dcData } });
  // }
  ...
  const fetchPartyById = async () => {
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}/api/telecaller/addParty/getPartyById/${dcData.adId}`);
      if (response.ok) {
  ...
  const fetchTableData = async () => {
    try {
      const response = await fetch(`${baseUrl}/api/telecaller/itemSalesTable/getfinalItems/${dcData.fId}`); // Replace with your API endpoint
      if (response.ok) {
        const data = await response.json();
  ...
  const fetchAmountsData = async () => {
    try {
      const response = await fetch(`${baseUrl}/api/telecaller/itemsTotal/getTotals/${dcData.fId}`); // Replace with your API endpoint
      if (response.ok) {
        const data = await response.json();
  ...
  const handleConvertToInvoice = async () => {
    ...
    try {
      ...
      formData.append("grandTotal", dcData.grandTotal); // Add the grandTotal as form data
  
      // Send the quotation data to convert to invoice
      const response = await fetch(
        `${baseUrl}/api/telecaller/invoice/save/${dcData.fId}`, // Replace with your endpoint
        {
          method: "POST",
      ...
      >
        ...
        <Box display="flex"  sx={{ marginBottom: '20px' }}>
          <ArrowBackIcon
            ...
            onClick={ () => navigate("/AccountsDCList")}
          />
          <Typography
            variant="body1"
            sx={{
          ...
          >
            Delivery Challan #{' '}
            <span style={{ color: '#1976d2' }}>{dcData.dcNumber}</span>
          </Typography>
        </Box>
                        ...
                        {/* <Grid item>
                          <Grid container justifyContent="space-between">
                            ...
                            >
                               ...
                               <strong>{dcData.quotationDate}</strong>
                            </Typography>
                          </Grid>
                        </Grid> */}
                        {/* <Grid item>
...
export default AccountsTelecallerDCTemplate;