...
export default function Accounts_UnPaidList() {
  ...
  const fetchQuotations = async () => {
    try {
      const response = await fetch(`${baseUrl}/api/transaction/getHistory/${sId}`);
      ...
      <CardContent>
        ...
        <TableContainer component={Paper}>
          <Table style={{ borderCollapse: "collapse" }}>
            <TableHead>
              <TableRow>
                {["S.No", "Contact Person Name", "Invoice Number", "Due Date", "Payment Date", "Invoice Amount", "UnPaid", "Payment Method","Transaction ID"].map((header) => (
                  <TableCell key={header} style={{ fontWeight: "bold", border: "1px solid #ACB4AE", fontSize: "17px", textAlign: "center", backgroundColor: "#ffebee" }}>{header}</TableCell>
                ))}
              </TableRow>
            </TableHead>
            <TableBody>
              {filteredQuotations.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, index) => (
                <TableRow key={index} hover>
                  ...
                  <TableCell style={{ border: "1px solid #ACB4AE", fontSize: "15px", textAlign: "center" }}>{row.unPaid ? row.unPaid : "Null"}</TableCell>
