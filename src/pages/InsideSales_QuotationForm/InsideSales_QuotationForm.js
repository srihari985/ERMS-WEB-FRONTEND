...
const InsideSales_QuotationForm = () => {
  ...
  const fetchPartyById = async () => {
    ...
    try {
      const response = await fetch(`http://192.168.29.219:8083/api/addparty/getparty/${selectedParty.adId}`);
      if (response.ok) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`http://192.168.29.219:8083/api/form/save/${selectedParty.adId}/${EmployeeRegId}`, {
        method: 'POST',
        headers: {
  ...
  const handleAddItemClick = () => {
    navigate('/InsideSales_ItemTable');
  };
      ...
      <Grid container alignItems="center" justifyContent="space-between">
        <Grid container alignItems="center" spacing={2}>
          <Grid item container alignItems="center" xs sx={{ marginTop: '10px' }}>
            <IconButton onClick={() => navigate('/InsideSales_QuotationList')} sx={{ color: '#2e2a54' }}>
              <KeyboardBackspaceIcon fontSize="inherit" />
            </IconButton>
            <Typography variant="h4" gutterBottom sx={{ marginTop: '7px' }}>
              Inside Sales Quotation Form
            </Typography>
          </Grid>
        </Grid>
      </Grid>
