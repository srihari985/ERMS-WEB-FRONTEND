...
const SalesManagerTechnicianRegister = () => {
      ...
      try {
        ...
        switch (values.role) {
          case 'OPERATION_TECH_LEAD':
              apiEndpoint = `${baseUrl}/api/v1/saleManagement/register/operationTechLead/${userId}`;
              break;
          case 'SALES':
              apiEndpoint = `${baseUrl}/api/v1/saleManagement/register/sales/${selectedId}`;
              break;          
          case 'TECHNICIAN':
              apiEndpoint = `${baseUrl}/api/v1/saleManagement/register/technician/${selectedId}`;
              break;
          case 'TELECALLER':
              apiEndpoint = `${baseUrl}/api/v1/saleManagement/register/telecaller/${selectedId}`;
              break;
          default:
...
const fetchDataForRole = async (role) => {
  try {
    ...
    if (role === 'SALES' || role === 'TELECALLER' || role === 'TECHNICIAN') {
      ...
      response = await fetch(`${baseUrl}/api/v1/saleManagement/getListOf/OpTechLead/ids`);
      if (!response.ok) {
...
export default SalesManagerTechnicianRegister;