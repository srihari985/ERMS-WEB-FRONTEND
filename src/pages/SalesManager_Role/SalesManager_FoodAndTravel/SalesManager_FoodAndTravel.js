...
const SalesManager_FoodTravelAllowance = () => {
  ...
  const smId = localStorage.getItem("smId");
  ...
  const [foodsearchDate, setFoodSearchDate] = useState('');
   ...
   const fetchTravelAllowanceData = async () => {
    try {
      ...
      const response = await fetch(`${baseUrl}/api/saleManager/travelAllowance/getAll`);
  ...
  const fetchFoodAllowanceData = async () => {
    try {
      const baseUrl=process.env.REACT_APP_API_BASE_URL
      const response = await fetch(`${baseUrl}/api/saleManager/foodAllowance/getAll`);
      const data = await response.json();                     
  ...
  const handleTravelSubmit = async (values, { setSubmitting, resetForm }) => {
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}/api/saleManager/travelAllowance/save/${smId}`, {
        method: 'POST',
        body: formData,
      });
  ...
  const handleFoodSubmit = async (values, { setSubmitting, resetForm }) => {
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}/api/saleManager/foodAllowance/save/${smId}`, {
        method: 'POST',
...
export default SalesManager_FoodTravelAllowance;