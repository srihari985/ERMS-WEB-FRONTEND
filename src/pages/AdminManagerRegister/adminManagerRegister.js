  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/api/v1/admin/register/manager/${userId}?firstname=${values.firstname}&lastname=${values.lastname}&email=${values.email}&role=${values.role}&password=${values.password}`,
        {
          method: 'POST',
          headers: {
